...
The package includes schema adapter implementations for the following database types:

- **MySQL / MariaDB** → `JardisCore\DbSchema\adapter\MySql`
- **PostgreSQL** → `JardisCore\DbSchema\adapter\Postgres`
- **SQLite** → `JardisCore\DbSchema\adapter\SqLite`

Each adapter class implements `JardisPsr\DbSchema\DbSchemaInterface` and handles database-specific schema queries, returning results in consistent, normalized structures.
...
All adapter classes implement `JardisPsr\DbSchema\DbSchemaInterface`:
...
The package follows the adapter pattern with database-specific implementations that handle:
